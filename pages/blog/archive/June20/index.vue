  ...
  <div>
    ...
    <ul class="links">
      <li v-for="info in monthes" :key="info" style="display: inline-block">      
        <nuxt-link  v-if="info.to=='June20'" class="nowMenu" :to= "{path:`/blog/archive/${info.to}`}" >{{ info.title }}({{info.blogCount}})</nuxt-link>
        <nuxt-link  v-else class="menu" :to= "{path:`/blog/archive/${info.to}`}" >{{ info.title }}({{info.blogCount}})</nuxt-link>
      </li>
    </ul>

    <ul  class="links">  
      <div class="new">2020年6月</div>
...
export default {
  ...
  computed: {
    ...
    preMonth:function(){
      for(var i=0;i<this.$store.getters['monthes'].length;i++){
        if(this.$store.getters['monthes'][i].to=="June20"){
